...
function long_scalar_mult_200(n, k, a, b) {
    var out[200];
    for (var i = 0; i < 200; i++) {
        out[i] = 0;
    }
...
function long_div(n, k, m, a, b){
    ...
    for (var i = m; i >= 0; i--) {
        ...
        remainder = long_sub_200(n, m + k, remainder, subtrahend);
    }
    for (var i = 0; i < k; i++) {
        out[1][i] = remainder[i];
    }
...
function short_div(n, k, a, b) {
   ...
   // k + 2 registers now
   var norm_a[200] = long_scalar_mult_200(n, k + 1, scale, a);
   // k + 1 registers now
   var norm_b[200] = long_scalar_mult_200(n, k, scale, b);
